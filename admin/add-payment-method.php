...
if(isset($_POST['submit'])){
    ...
    $check = _fetch("payment","pmn_method='$method_name'");
    if($check){
        header("location:payment-method.php?err=Already exist this Method");
    }else{
      $insert = _insert("payment","pmn_method,description,status","'$method_name','$description','$status'");
      if($insert){
        $msg = "Successfully Inserted";
        header("Location:add-payment-method.php?msg=$msg");
      }else{
        $err = "Something is error!";
        header("Location:add-payment-method.php?err=$err");
      }
    }
      ...
      <div class="x_container space-y-10 py-10">
        ...
        <div class="grid grid-cols-2 gap-y-8 gap-x-12">
          <div class="col-span-2">
            <h2 class="text-xl font-semibold text-cyan-800">Add Payment Method</h2>
          </div>
        </div>
        <br>
        ...
        <div>        
          <div class="col-span-2 lg:col-span-1 flex flex-col gap-y-1">
            ...
            <input name="method_name" class="input" type="text" id="method_name" placeholder="Payment Method Name" required>
          </div>
          <br>
